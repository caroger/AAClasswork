...
function stepper(nums) {
  ...
  for (let i = 0; i < nums.length; i++) {
    if (table[i] === true) {
      let maxRange = nums[i];
      for (let j = 1; j <= maxRange && i + j < nums.length; j++) {
        table[i + j] = true;
      }
    }
  }
...
function maxNonAdjacentSum(nums) {
  ...
  table = new Array(nums.length).fill(0);

  table[0] = nums[0];

  for (let i = 1; i < nums.length; i++) {
    let skipLeft = table[i - 2] || 0;
    let includeCurrent = skipLeft + nums[i];
    let skipCurrent = table[i - 1];
    table[i] = Math.max(includeCurrent, skipCurrent);
  }
...
function minChange(coins, amount) {
  ...
  for (let i = 1; i < table.length; i++) {
    ...
    coins.forEach((coin) => {
      if (i - coin >= 0) {
        possibility.push(table[i - coin] );
      }
      // console.log(`finding change for ${i}`);
      // console.log(
      //   `if using coin ${coin}, min number coins required at ${
      //     i - coin || 0
      //   } is ${table[i - coin] || 0}`
      // );
    });
    table[i] = Math.min(...possibility) + 1;
    // console.log(table);
  }
  return table[amount];
}
